...
def add_venue(name, city):
    venue, _ = Venue.objects.get_or_create(name=name, defaults={"city": city})
    return venue


def add_artist(name, genres, spotify_image, external_id):
    ...
    if artist.spotify_image != spotify_image:
        artist.spotify_image = spotify_image
        changed = True
...
def add_song(name, artist, spotify_id):
    song, _ = Song.objects.get_or_create(
        name=name, artist=artist, defaults={"external_id": spotify_id}
    )
